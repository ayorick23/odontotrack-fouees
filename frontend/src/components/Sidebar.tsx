...
export function Sidebar({ user }: { user: AuthUser | null }) {
  if (user === null) {
    return (
      <aside className="flex w-64 shrink-0 flex-col rounded-2xl bg-white p-4 shadow-sm">
  ...
  return (
    <aside className="flex w-64 shrink-0 flex-col rounded-2xl bg-white p-4 shadow-sm">
      <div className="mb-6 flex items-center gap-3 px-1">
        ...
        />
        <div>
          <p className="text-sm font-semibold text-slate-800">FOUEES</p>
    ...
    <div>
      <button
        ...
        className={`flex w-full items-center gap-2 rounded-xl px-3 py-2 text-left text-sm font-medium ${
          childActive ? "bg-slate-100 text-slate-800" : "text-slate-600 hover:bg-slate-50"
        }`}
      >
      ...
      {open ? (
        <div className="mt-1 ml-4 flex flex-col gap-1 border-l border-slate-100 pl-2">
          {children.map((child) => (
            <SidebarLink
              key={child.to}
              to={child.to}
